...
const StandingsTable = (props) => {
  ...
  const compName = route.pathname.split('/')[2]
  const category = route.pathname.split('/')[1]
          ...
          <table className="min-w-full text-center text-sm font-light">
            <thead className="border-b font-medium">
              <tr>
                ...
                {compName === 'top14' && <th scope="col" className="px-6 py-4 text-lg border-r">C.E</th>}
                <th scope="col" className="px-6 py-4 text-lg border-r">Match Joués</th>
                <th scope="col" className="px-6 py-4 text-lg border-r">Victoires - Egalités - Défaites</th>
                {category === "rugby" && <th scope="col" className="px-6 py-4 text-lg border-r">{`Bonus ${props.type === 'b' ? ('(Offensif + Défensif)') : ''}`}</th>}
