const Select = (props) => {
  ...
  return (
    <div className="flex flex-row items-center text-center justify-around gap-4">
      <div 
        className={`element border w-full p-5 text-xl border-b-0 ${props.disabled ? 'cursor-not-allowed' : 'cursor-pointer'} hover:bg-black ${props.currentTab === "Teams" ? "bg-black" : ""}`} 
      ...
      <div 
        className={`element border w-full p-5 text-xl border-b-0 ${props.disabled ? 'cursor-not-allowed' : 'cursor-pointer'} hover:bg-black ${props.currentTab === "Matches" ? "bg-black" : ""}`} 
        onClick={() => props.disabled == false && props.setCurrentTab("Matches")}
      >Prochains matchs</div>
      {props.live && (
        <div className={`element border w-full p-5 text-xl border-b-0 ${props.disabled ? 'cursor-not-allowed' : 'cursor-pointer'} hover:bg-black ${props.currentTab === "Live" ? "bg-black" : ""}`} 
